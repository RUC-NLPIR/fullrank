...
def dfs_reachability(reachable):
    nodes = list(reachable.keys())
    for node in nodes:
        visited = set()
        def dfs(current):
            for neighbor in reachable[current]:
                if reachable[current][neighbor] and neighbor not in visited:
                    visited.add(neighbor)
                    dfs(neighbor)
        dfs(node)
        # 更新节点之间的可达性
        for other_node in visited:
            reachable[node][other_node] = True
    return reachable
...
class RankLLM(ABC):
    ...
    def permutation_pipeline(
        ...
        output_passages_num = rank_end - rank_start
        # output_passages_num = 11
        time_start = time.time()
        if self._model in ['gpt-4o-mini', 'gpt-4o-2024-08-06']:
            permutation, in_token_count, out_token_count = self.run_llm(prompt, output_passages_num=output_passages_num)
        else:
            permutation, out_token_count = self.run_llm(prompt, output_passages_num=output_passages_num)
        time_cost = time.time() - time_start
        rerank_detail = {
    ...
    def permutation_pipeline_baidubatch(
        self,
        result: Result,
        rank_start: int,
        rank_end: int,
        ...
        """
        prompt, in_token_count = self.create_prompt(result, rank_start, rank_end)
        shard = self.processed_queries // 500
        with open(f'../gpt_batch/gpt-4o-2024-08-06_data_{shard}.jsonl', 'a') as f:
            data = {
                "custom_id": result.query.qid, 
                "method": "POST", 
        ...
        #     prompt, output_passages_num=rank_end - rank_start
        # )
        permutation, out_token_count = '', 500
        time_cost = time.time() - time_start
    ...
    def sliding_windows(
        ...
        #     compare_dict[candidate.docid] = {}
        #     for candidate1 in rerank_result.candidates:
        #         compare_dict[candidate.docid][candidate1.docid] = False 
        # error_flag = copy.deepcopy(compare_dict) # -------------------------------
        total_time_cost = 0
        rerank_details = []
        while windows_end > rank_start and windows_start + step != rank_start:
            windows_start = max(windows_start, rank_start)
            ...
            # # --------------------------
            # for i in range(windows_start, windows_end):
            #     docid1 = rerank_result.candidates[i].docid
            #     for j in range(i + 1, windows_end):
            #         docid2 = rerank_result.candidates[j].docid
            #         compare_dict[docid1][docid2] = True
            # # for i in range(windows_start, (windows_start + windows_end) // 2):
            # #     docid1 = rerank_result.candidates[i].docid
            # #     for j in range(windows_end, rank_end):
            # #         docid2 = rerank_result.candidates[j].docid
            # #         compare_dict[docid1][docid2] = True
            # # --------------------------
            windows_end = windows_end - step
        ...
        # for doc_from, doc_tos in compare_dict.items():
        #     for doc_to in doc_tos:
        #         if compare_dict[doc_from][doc_to]:
        #             total += 1
        #         if compare_dict[doc_from][doc_to] and compare_dict[doc_to][doc_from]:
        #             error += 1
        #             error_flag[doc_from][doc_to] = error_flag[doc_to][doc_from] = True
        # print(error, total, error / total)
        # # remove error
        # for doc_from, doc_tos in compare_dict.items():
        #     for doc_to in doc_tos:
        #         if compare_dict[doc_from][doc_to] and compare_dict[doc_to][doc_from]:
        #             compare_dict[doc_from][doc_to] = compare_dict[doc_to][doc_from] = False
        
        # total1 = 0
        # error1 = 0
        # error2 = 0
        # # extend graph
        # compare_dict = floyd_warshall(compare_dict)
        # for doc_from, doc_tos in compare_dict.items():
        #     for doc_to in doc_tos:
        #         # if doc_from != doc_to and compare_dict[doc_from][doc_to] == 0 and compare_dict[doc_to][doc_from] == 0:
        #         #     flag += 1
        #         if compare_dict[doc_from][doc_to]:
        #             total1 += 1
        #         if compare_dict[doc_from][doc_to] and compare_dict[doc_to][doc_from]:
        #             error1 += 1
        #         if compare_dict[doc_from][doc_to] and error_flag[doc_from][doc_to]:
        #             error2 += 1
        # print(error1, total1, error1 / total1, error2)
        # --------------------------
    ...
    def receive_permutation(
        ...
        for j, x in enumerate(response): # lwh
            ...
            if result.candidates[rank_start + j].score:
